...
const DATASET_ROOT = path.resolve(PROJECT_ROOT, "bpss_agentic_dataset");
const DATA_DIR = path.resolve(PROJECT_ROOT, "data");
const CACHE_DIR = path.resolve(PROJECT_ROOT, ".cache");
const DEFAULT_USER_ID = "local-user";
const DEFAULT_SESSION_ID = "bpss-demo";
...
async function initializeUploadAndWait(input) {
...
async function searchMarkdown(input) {
...
async function getPreviewMarkdown(input) {
  return fs.readFile(sheetPath(input.userId, input.sessionId, input.fileId, input.sheetId, "preview.md"), "utf8");
}
...
const mastraDbUrl = `file:${databasePath}`;
...
"use strict";
const documentAnalystAgent = new Agent({
...
"use strict";
const agentKeys = ["coordinatorAgent", "documentAnalystAgent", "researchAgent"];
